...
This project uses [Beads (bd)](https://github.com/steveyegge/beads) for issue tracking. Run `bd prime` to load workflow context, or `bd help` for commands.
...
- Read `context.md`, then run `bd ready` to see available work.
...
- Update bd task status as you work (`bd start <id>`, `bd close <id>`).
